...
export function setMark(token, taskId, userId, mark){
  return axios({
    method: 'post',
    url: BASE_URL + '/tasks/setMark',
    headers: {
      "Authorization": "Bearer " + token,
      "Content-Type": "application/json"
    },
...
export function getProjectStructure(token, taskId, userId){
  return axios({
    method: 'get',
    url: BASE_URL + '/project/getProjectStructure?taskId=' + taskId + '&userId=' + userId,
    headers: { "Authorization": "Bearer " + token }
  })
}

export function fetchFile(token, fileId){
  return axios({
    method: 'get',
    url: BASE_URL + '/project/getFile?fileId=' + fileId,
    headers: { "Authorization": "Bearer " + token }
  })
}
